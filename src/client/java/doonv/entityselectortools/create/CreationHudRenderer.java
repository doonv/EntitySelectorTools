    ...
    public static void renderHints(GuiGraphicsExtractor guiGraphics) {
        ...
        if (CreationManager.pos1 == null) {
            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.translatable("entityselectortools.box-create.hud.pos1",
                            Component.keybind("key.attack")), x,
                    y, color);
        } else if (CreationManager.pos2 == null) {
            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.translatable("entityselectortools.box-create.hud.pos2", Component.keybind("key.use")),
                    x,
                    y, color);
        } else {
            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.translatable("entityselectortools.box-create.hud.extend",
                            Component.keybind("key.pickItem")), x,
                    y, color);
            y += 10;
            guiGraphics.centeredText(Minecraft.getInstance().font,
                    Component.translatable("entityselectortools.box-create.hud.copy",
                            EntitySelectorToolsClient.COPY_KEY.getTranslatedKeyMessage()), x, y, color);
        }
        //~}
    }
}
